...
export default function App() {
  const [currentUser, setCurrentUser] = useState(() => {
    try {
      ...
      if (savedUser && savedToken) {
        try {
          // Basic token validation - check if token structure is valid
  ...
  useEffect(() => {
    ...
    // Menu categories realtime events
    socket.on('menuCategory:created', (category) => {
      const id = normalizeId(category.id)
    ...
    socket.on('menuCategory:updated', (category) => {
      const id = normalizeId(category.id)
      setMenuCategories((prev) => {
        const exists = prev.some((item) => normalizeId(item.id) === id)
        const updated = prev.map((item) => (normalizeId(item.id) === id ? category : item))
        return exists ? updated : [...updated, category]
      })
    })
    socket.on('menuCategory:deleted', (id) => {
      const normalizedId = normalizeId(id)
      setMenuCategories((prev) => prev.filter((item) => normalizeId(item.id) !== normalizedId))
    })

    // Menu items realtime events
    socket.on('menuItem:created', (item) => {
      const id = normalizeId(item.id)
      setMenuItems((prev) => {
        if (prev.some((i) => normalizeId(i.id) === id)) return prev
        return [...prev, item]
      })
    })
    socket.on('menuItem:updated', (item) => {
      const id = normalizeId(item.id)
      setMenuItems((prev) => {
    ...
    socket.on('menuItem:deleted', (id) => {
      const normalizedId = normalizeId(id)
      setMenuItems((prev) => prev.filter((item) => normalizeId(item.id) !== normalizedId))
    })
  ...
  async function handleUpdateBooking(id, bookingData) {
    ...
    const normalizedId = normalizeId(id)
    setBookings((prev) => prev.map((entry) => (normalizeId(entry.id) === normalizedId ? updatedBooking : entry)))
  }
  ...
  // Menu handlers
  async function handleAddMenuCategory(category) {
    const newCategory = await menuCategoriesAPI.create(category)
    const newCategoryId = normalizeId(newCategory.id)
    setMenuCategories((prev) => {
      if (prev.some((entry) => normalizeId(entry.id) === newCategoryId)) {
        return prev
  ...
  async function handleEditMenuCategory(id, category) {
    await menuCategoriesAPI.update(id, category)
    const normalizedId = normalizeId(id)
    setMenuCategories((prev) => prev.map((entry) => (normalizeId(entry.id) === normalizedId ? { ...entry, ...category } : entry)))
  }

  async function handleDeleteMenuCategory(id) {
    await menuCategoriesAPI.delete(id)
    const normalizedId = normalizeId(id)
    setMenuCategories((prev) => prev.filter((entry) => normalizeId(entry.id) !== normalizedId))
  }
  ...
  async function handleAddMenuItem(item) {
    ...
    setMenuItems((prev) => {
      if (prev.some((entry) => normalizeId(entry.id) === newItemId)) {
        return prev
      }
      return [...prev, newItem]
    })
  ...
  async function handleEditMenuItem(id, item) {
    await menuItemsAPI.update(id, item)
    const normalizedId = normalizeId(id)
    setMenuItems((prev) => prev.map((entry) => (normalizeId(entry.id) === normalizedId ? { ...entry, ...item } : entry)))
  }

  async function handleDeleteMenuItem(id) {
    await menuItemsAPI.delete(id)
    const normalizedId = normalizeId(id)
    setMenuItems((prev) => prev.filter((entry) => normalizeId(entry.id) !== normalizedId))
  }
