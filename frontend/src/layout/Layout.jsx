...
const Sidebar = memo(({ mobile = false, currentPage, onNavigate, onCloseSidebar, onToggleCollapse, sidebarCollapsed, userRole, userName, onLogout, isNavigating }) => {
  ...
  return (
    ...
    >
      ...
      <div className={`flex items-center gap-3 border-b border-gray-800 relative ${sidebarCollapsed && !mobile ? 'justify-center px-0 py-4' : 'px-6 py-5'}`}>
        ...
        {mobile && (
          <button onClick={onCloseSidebar} className="ml-auto text-slate-400 hover:text-white">
            <X size={18} />
          </button>
        )}
        {!mobile && (
